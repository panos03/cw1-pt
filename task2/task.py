...
def save_robustness_demo(test_loader, filename, alpha=0.4):
    ...
    shows one MixUp output, illustrating the blending of two source images.
    ...
    mixed_x, _, lam = mixup(images[:n * 2], y_onehot, alpha=alpha)
    mixed_x = mixed_x[:n]

    # Build 4x4 grid
    scale = 2
    cell = 28 * scale   # 56 px per image
    border = 3
    grid = 4
    header_h = 20
    canvas_w = grid * cell + (grid + 1) * border
    canvas_h = grid * cell + (grid + 1) * border + header_h

    canvas = Image.new("RGB", (canvas_w, canvas_h), (230, 230, 230))
    draw = ImageDraw.Draw(canvas)
    draw.text((5, 4), f"MixUp Montage  lam={lam:.2f}  alpha={alpha}", fill=(40, 40, 40))

    for i in range(n):
        row = i // grid
        col = i % grid
        x_off = border + col * (cell + border)
        y_off = header_h + border + row * (cell + border)
...
def main():
    ...
    sigma = 0.3
    noisy_acc = evaluate_noisy(model, test_loader, sigma=sigma)
